...
public class CalendarStepDefinitions {
    ...
    public void userCanDeleteAny(String event) {
        ...
        try {
            WebElement event1 = Driver.getDriver().findElement(By.xpath(xpath));
            Assert.fail("Event is not deleted");
    ...
    public void userSeeUpdated(String categoryName) {
       BrowserUtils.sleep(1);
        String xpath = "//span[@class = 'multiselect__tag']//span[text()='"+categoryName+"']";
        WebElement updatedCategory = Driver.getDriver().findElement(By.xpath(xpath));
     // System.out.println(updatedCategory.toString());
       // System.out.println();

        Assert.assertTrue(updatedCategory.getText().equalsIgnoreCase(categoryName));
